...
struct TreeNode *searchBST(tree t,char*  x) {
  ...
  if (strcmp(value(t).email,x) == 0)
    return t;
  else if (strcmp(value(t).email,x) < 0)
...
tree insertBST(tree t, element_t x) {
  ...
  if (strcmp(value(t).email,x.email) == 0)
    return t;
  else if (strcmp(value(t).email,x.email)>0) {
...
void inorderTravel(tree t){
  ...
  if (isLeaf(t)) {
    printf("%s %s %s ",value(t).name,value(t).num,value(t).email);
    return;
  }
...
tree readData(tree t,char* filename) {
  ...
  while (fgets(buf, 70, fin)) {
    info new;
    sscanf(buf," %s %s %s",new.name,new.num,new.email);
    printf("%s %s %s\n", new.name,new.num,new.email);
    t = insertBST(t,new);
  }
  return t;
}
